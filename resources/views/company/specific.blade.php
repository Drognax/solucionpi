...
@section('boton')
<a data-toggle="modal" data-target="#modalItem">Nuevo Documento </a>
          ...
          <div class="modal fade" id="exampleModal{{$loop->index}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            ?>                                   
              <td>
			    ...
			    <td>
              <form method="POST" action="{{URL('/especifica/delete')}}" id="eliminar">
                    {{ csrf_field() }}
                    {{method_field('PUT')}}
                <input type="hidden" id="id" name="id" value="{{ $doc->id}}">
                <input type="hidden" id="act" name="act" value="{{ $doc->actividad}}">
